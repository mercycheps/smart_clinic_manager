// frontend/src/components/dashboards/LabtechDashboard.jsx
...
const LabtechDashboard = () => {
  ...
  const fetchAssignedTests = async () => {
    try {
      const res = await axios.get(`${API_URL}/labtech/assigned`, { headers });
  ...
  const handleSubmit = async (e, resultId) => {
    ...
    try {
      await axios.post(`${API_URL}/labtech/record`, {
        result_id: resultId,
        results
      }, { headers });
      setMessage('✅ Lab result submitted successfully.');
      setResultInputs(prev => ({ ...prev, [resultId]: '' }));
      fetchAssignedTests();
    } catch (err) {
  ...
  return (
    <div className="container">
      ...
      {message && (
        <p style={{ color: message.includes('✅') ? 'green' : 'red', marginTop: '1rem' }}>
          {message}
        </p>
      )}
    </div>
